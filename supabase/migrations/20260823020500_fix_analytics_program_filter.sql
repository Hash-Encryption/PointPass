...
begin
  ...
  with authorized_branches as (
    ...
    where t.business_id = _business_id
      ...
      and (_program_type is null or t.program_type::text = _program_type)
    ...
    from (
      select coalesce(program_type::text, 'legacy') program_type,
        count(*) total,
        count(*) filter (where action in ('stamp', 'points')) earns,
        count(*) filter (where action = 'redeem') redemptions
      from filtered group by coalesce(program_type::text, 'legacy')
    ) grouped
  ), branch_activity as (
    select coalesce(jsonb_agg(jsonb_build_object(
      'id', branch_id,
